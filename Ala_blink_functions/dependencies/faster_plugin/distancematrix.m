...
for chan2tst = eeg_chans;
	for q=eeg_chans
		distmatrixpol(chan2tst,q) =...
        sqrt(...
            ((EEG.chanlocs(chan2tst).radius^2)+(EEG.chanlocs(q).radius^2)) - ...
            ( 2*(...
                (EEG.chanlocs(chan2tst).radius) * ...
                (EEG.chanlocs(q).radius) * ...
                cosd(EEG.chanlocs(chan2tst).theta - EEG.chanlocs(q).theta)...
                )...
             )...
         );%calculates the distance between electrodes using polar format
	end
end
