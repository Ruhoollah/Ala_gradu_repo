function [mu, sigma, P] = EMgauss1D(x, indices, make_plot)
...
% If make_plot==1 --> the estimation procedure is plotted along the iterations
...
if make_plot, if num2str(length(find(isnan(p))))>0, disp(['  --- N of NaN indices: ' , num2str(length(find(isnan(p))))]); end, end
