function [msg, fun, args] = check_ctap_hist(eeg, outdir)
%CHECK_CTAP_HIST displays the history struct from a CTAP-processed EEG file
%
% Description:
...
elseif ~isstruct(eeg)
    error('This is not an EEG struct');
elseif ~isfield(eeg, 'CTAP')
    error('There is no CTAP field in EEG ');
end
