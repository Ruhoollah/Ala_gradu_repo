function [EEG, Cfg] = CTAP_fir_filter(EEG, Cfg)
%CTAP_fir_filter - FIR filter data using firfilt plugin (pop_eegfiltnew.m)
%
% Description:
...
%
% Syntax:
%   [EEG, Cfg] = CTAP_fir_filter(EEG, Cfg);
%
% Inputs:
...
%                           allowed values: {0,1}, default: 0 (bandpass)
...
if isempty(Arg.locutoff) && isempty(Arg.hicutoff)
   error('CTAP_fir_filter:inputError', ...
       'At least one of the filter cut off frequencies ''locutoff'' or ''hicutoff'' needs to be set. See pop_eegfiltnew.m for details.'); 
end
...
msg = sprintf('FIR filtered data using pop_eegfiltnew(): %s', EEG.setname); 
...
if Cfg.grfx.on
    ...
    qcfile = fullfile(get_savepath(Cfg, mfilename, 'qc'),...
          sprintf('filter_effects_%s.png', EEG.CTAP.measurement.casename));
    saveas(fg, qcfile)
    close(fg);
    ...
    qcfile = fullfile(get_savepath(Cfg, mfilename, 'qc'),...
          sprintf('filter_properties_%s.png', EEG.CTAP.measurement.casename));
    saveas(fg, qcfile)
    close(fg);
